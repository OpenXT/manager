...
newtype Rpc a = Rpc (RpcM DiskSyncError a)
    deriving (Monad, MonadError DiskSyncError, MonadRpc DiskSyncError, FreezeIOM RpcContext (Either DiskSyncError))
...
instance MonadIO Rpc where
    liftIO act =  Rpc $
        do status <- liftIO $ E.try act
           ...
           tunnel status
        where
          tunnel :: Either E.SomeException a -> RpcM DiskSyncError a
