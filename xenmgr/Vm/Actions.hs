...
startupCheckDiskPaths uuid
  = do disks <- M.elems <$> liftRpc (getDisks uuid)
       goodPaths <- filterM checkPath disks
       if (length goodPaths) /= (length disks)
         then do error $ "Bad Physical Path for one or more disks for VM " ++ show uuid
         else do return True
    where
      checkPath disk = liftIO $ doesFileExist $ diskPath disk
